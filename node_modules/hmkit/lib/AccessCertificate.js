...
var AccessCertificate = function () {
  ...
  _createClass(AccessCertificate, [{
    ...
    value: function parse(bytes) {
      var unparsedBytes = [].concat(_toConsumableArray(bytes));

      var response = {
        version: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 1)).toUpperCase(),
        issuer: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 4)).toUpperCase(),
        accessProvidingSerialNumber: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 9)).toUpperCase(),
        accessGainingSerialNumber: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 9)).toUpperCase(),
        accessGainingPublicKey: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 64)).toUpperCase(),
        validityStartDate: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 5)).toUpperCase(),
        validityEndDate: (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 5)).toUpperCase()
      };

      var permissionsSize = (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 1)).toUpperCase();
      var permissions = (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, (0, _encoding.hexToInt)(permissionsSize))).toUpperCase();
      var signature = (0, _encoding.uint8ArrayToHex)(unparsedBytes.splice(0, 64)).toUpperCase();
