...
var ChassisSettingsResponse = function (_PropertyResponse) {
  ...
  function ChassisSettingsResponse(data, config) {
    ...
    var properties = [new PropertyDecoder(0x01, 'drivingMode').setDecoder(switchDecoder({
      0x00: 'regular',
      0x01: 'eco',
      0x02: 'sport',
      0x03: 'sport_plus',
      0x04: 'eco_plus'
    })), new PropertyDecoder(0x02, 'sportChrono').setDecoder(switchDecoder({
      0x00: 'inactive',
      0x01: 'active'
    })), new PropertyDecoder(0x05, 'currentSpringRates').setOptionalSubProperties('axle', [new OptionalPropertyDecoder(0x00, 'front_axle').setDecoder(_this.axleDataDecoder('springRate')), new OptionalPropertyDecoder(0x01, 'rear_axle').setDecoder(_this.axleDataDecoder('springRate'))]), new PropertyDecoder(0x06, 'maximumSpringRates').setOptionalSubProperties('axle', [new OptionalPropertyDecoder(0x00, 'front_axle').setDecoder(_this.axleDataDecoder('springRate')), new OptionalPropertyDecoder(0x01, 'rear_axle').setDecoder(_this.axleDataDecoder('springRate'))]), new PropertyDecoder(0x07, 'minimumSpringRates').setOptionalSubProperties('axle', [new OptionalPropertyDecoder(0x00, 'front_axle').setDecoder(_this.axleDataDecoder('springRate')), new OptionalPropertyDecoder(0x01, 'rear_axle').setDecoder(_this.axleDataDecoder('springRate'))]), new PropertyDecoder(0x08, 'currentChassisPosition').setDecoder(function (bytes) {
      return uint8toInt8(bytes[0]);
    }), new PropertyDecoder(0x09, 'maximumChassisPosition').setDecoder(function (bytes) {
      return uint8toInt8(bytes[0]);
    }), new PropertyDecoder(0x0a, 'minimumChassisPosition').setDecoder(function (bytes) {
      return uint8toInt8(bytes[0]);
    })];
  ...
  _createClass(ChassisSettingsResponse, [{
    ...
    value: function axleDataDecoder(namespace) {
      return function (bytes) {
        return _defineProperty({}, namespace, uint8toInt8(bytes[0]));
      };
    }
  }]);
...
export default ChassisSettingsResponse;