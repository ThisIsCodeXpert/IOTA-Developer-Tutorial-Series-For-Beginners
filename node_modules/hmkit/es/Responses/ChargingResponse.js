...
import { bytesSum, dateDecoder, getRoundedIeee754ToBase10, switchDecoder, progressDecoder } from '../helpers';
...
var ChargingResponse = function (_PropertyResponse) {
  ...
  function ChargingResponse(data) {
    ...
    var properties = [new Property(0x02, 'estimatedRange').setDecoder(bytesSum), new Property(0x03, 'batteryLevel').setDecoder(progressDecoder), new Property(0x04, 'batteryCurrentAC').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x05, 'batteryCurrentDC').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x06, 'chargerVoltageAC').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x07, 'chargerVoltageDC').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x08, 'chargeLimit').setDecoder(progressDecoder), new Property(0x09, 'timeToCompleteCharge').setDecoder(bytesSum), new Property(0x0a, 'chargingRateKW').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x0b, 'chargePortState').setDecoder(switchDecoder({
      0x00: 'closed',
      0x01: 'open'
    })), new Property(0x0c, 'chargeMode').setDecoder(switchDecoder({
      0x00: 'immediate',
      0x01: 'timer_based',
      0x02: 'inductive'
    })), new Property(0x0e, 'maxChargingCurrent').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x0f, 'plugType').setDecoder(switchDecoder({
      0x00: 'type_1',
      0x01: 'type_2',
      0x02: 'ccs',
      0x03: 'chademo'
    })), new Property(0x10, 'chargingWindowChosen').setDecoder(switchDecoder({
      0x00: 'not_chosen',
      0x01: 'chosen'
    })), new Property(0x11, 'departureTimes').array().setDecoder(_this.departureTimeDecoder), new Property(0x13, 'reductionTimes').array().setDecoder(_this.reductionTimeDecoder), new Property(0x14, 'batteryTemperature').setDecoder(getRoundedIeee754ToBase10(2)), new Property(0x15, 'timers').setOptionalSubProperties('timerType', [new OptionalProperty(0x00, 'preferred_start_time').setDecoder(_this.timerTimeDecoder), new OptionalProperty(0x01, 'preferred_end_time').setDecoder(_this.timerTimeDecoder), new OptionalProperty(0x02, 'departure_time').setDecoder(_this.timerTimeDecoder)]), new Property(0x16, 'pluggedIn').setDecoder(switchDecoder({
      0x00: 'disconnected',
      0x01: 'plugged_in'
    })), new Property(0x17, 'activeState').setDecoder(switchDecoder({
      0x00: 'not_charging',
      0x01: 'charging',
  ...
  _createClass(ChargingResponse, [{
    ...
    value: function timerTimeDecoder() {
      return {
        time: dateDecoder.apply(undefined, arguments)
      };
    }
  }, {
