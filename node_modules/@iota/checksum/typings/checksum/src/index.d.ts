import { Trytes } from '../../types';
export declare const errors: {
    INVALID_ADDRESS: string;
    INVALID_CHECKSUM: string;
    INVALID_TRYTES: string;
    INVALID_CHECKSUM_LENGTH: string;
};
/**
 * Generates and appends the 9-tryte checksum of the given trytes, usually an address.
 *
 * @method addChecksum
 *
 * @param {string | string[]} input - Input trytes
 *
 * @param {number} [checksumLength=9] - Checksum trytes length
 *
 * @param {boolean} [isAddress=true] - Flag to denote if given input is address. Defaults to `true`.
 *
 * @returns {string | string[]} Address (with checksum)
 */
export declare function addChecksum(input: Trytes, checksumLength?: number, isAddress?: boolean): Trytes;
export declare function addChecksum(input: ReadonlyArray<Trytes>, checksumLength?: number, isAddress?: boolean): ReadonlyArray<Trytes>;
/**
 * Removes the 9-trytes checksum of the given input.
 *
 * @method removeChecksum
 *
 * @param {string | string[]} input - Input trytes
 *
 * @return {string | string[]} Trytes without checksum
 */
export declare function removeChecksum(input: Trytes): Trytes;
export declare function removeChecksum(input: ReadonlyArray<Trytes>): ReadonlyArray<Trytes>;
/**
 * Validates the checksum of the given address trytes.
 *
 * @method isValidChecksum
 *
 * @param {string} addressWithChecksum
 *
 * @return {boolean}
 */
export declare const isValidChecksum: (addressWithChecksum: string) => boolean;
